...
from keras.models import load_model
model3 = load_model("model_cats_dags.h5")
...
# fonction predicton image cat end dog

# def Predict_dog_cat(filenames):
#     list=[]
#     image=[]
#     for filename in filenames:   
...
#         x = x[np.newaxis] 
#         images=list.append(x)  
#     image = np.array(images)
#     image = np.vstack((images))                              
#     classes=model3.predict(image)
#     preds=[]
#     for predection in classes:
#         preds.append(predection)
#         print("prediction dog_vs_cat",preds)
#     print("prediction",preds)   
#     return preds

# x = x[np.newaxis]
# preds=model3.predict(x)
# preds=list.append(preds)
def Predict_dog_cat(filenames):
    preds=[]
    for filename in filenames:   
        img = image.load_img("static/uploads/"+ filename ,target_size = (150,150))
...
@app.route('/dog_vs_cat', methods=['POST'])
def dog_vs_cat():
    if 'files[]' not in request.files:
        flash('No file part')
        return redirect(request.url)
    ...
    flash('Image est charger par succes')
    preds= Predict_dog_cat(file_names)
    Zipp=zip(file_names,preds)
    return render_template('dog_vs_cat.html', RESULTAT=Zipp )
